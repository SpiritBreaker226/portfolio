...
angular
  ...
  .controller("caruselFeatureCtrl", function (
    $scope,
    $document,
    ProjectCommService,
    ProjectPortfolioHttpService,
    doDisplayPortfolioDetails
  ) {
    ...
    $scope.setProjectIndex = function (intProjectIndex) {
      ...
      $document.scrollToElement(
        angular.element(document.getElementById("portfolio")),
        70
      );
    ...
    $scope.getNumberOfProjectBanners = function (intNumberOfProjectsToDisplay) {
      ...
      if (
        intNumberOfProjectsToDisplay <= 0 ||
        intNumberOfProjectsToDisplay > arrProjectsOnSite.length
      ) {
      ...
      for (
        var intIndex = intNumberOfProjectsToDisplay;
        intIndex > 0;
        intIndex--
      ) {
        var intRandomIndexNumber = Math.floor(
          Math.random() * arrProjectsOnSite.length
        );
